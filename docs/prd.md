...
* 技术词汇并非普通英语（coroutine / cache / daemon / enum）
* 技术文档是主要词源，但缺乏发音提示
* 社区长期形成错误读法并被传播
...
* Provider 轮询机制
  * `/api/phonetics` 支持 `auto` 模式，自动在 youdao/eudic 之间轮询
  * 避免单点依赖和限流
  * 前端默认使用 `auto`，无需感知具体 provider
...
* 封装有道词典 API
* 返回音标（`ipa_us`, `ipa_uk`）和音频 URL（`audio_url_us`, `audio_url_uk`）
* 包含 sign 签名计算

### 欧陆词典解析器 (`src/lib/eudic/parser.ts`)
...
### 后台管理 (`src/routes/admin/+page.svelte`)
